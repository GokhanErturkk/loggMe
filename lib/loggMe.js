...
// for some reason kafkajs gives a warning when trying to produce 10 message at once 
...
class loggMe {
    constructor() {
        ...
        this.logFormatType = process.env.LOGGME_FORMAT || 'dev'
     ...
     */
    createKafkaStream(brokers,topic, clientId) {
        this.stream = new KafkaLogger(brokers, topic, clientId)
        return this
    }
    /**
    ...
    logWrapper(logLevel) {
        return function log() {
            ...
            if (args[0] instanceof Error) {
                err = serializeErr(args[0], true, this.logFormatType)
            } else if (isString(args[0])) {
                logObj["msg"] = args[0]
            } else if (isObject(args[0]) && isString(args[1])) {
                logObj = { ...logObj, ...args[0], msg: args[1] }
